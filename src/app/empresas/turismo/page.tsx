...
function riskColor(score: number) {
  if (score >= 70) return { bar: "bg-red-500",    badge: "bg-red-900/20 text-red-400",       label: "Crítico"  };
  if (score >= 55) return { bar: "bg-orange-400", badge: "bg-orange-900/20 text-orange-400", label: "Alto"     };
  return              { bar: "bg-yellow-400",  badge: "bg-yellow-900/20 text-yellow-400",  label: "Moderado" };
}
...
const KPI = [
  { label: "Salario mediano sectorial",  valor: "$680.000",    sub: "sector con menor sueldo de Chile",        icon: TrendingUp,      color: "text-white"      },
  { label: "Rotación anual",             valor: "52%",         sub: "más alta del mercado laboral chileno",     icon: TrendingDown,    color: "text-red-400"    },
  { label: "Aumento dotación peak",      valor: "+65%",        sub: "diciembre–marzo y julio–agosto",           icon: AlertTriangle,   color: "text-amber-400"  },
  { label: "Costo reposición garzon",    valor: "$1.850.000",  sub: "por rotación incl. capacitación estándar", icon: CircleDollarSign,color: "text-orange-400" },
];
...
const IMPACTO_TOTAL = [
  { label: "Ahorro proyectado",      valor: "$69.460.000",  sub: "con plan completo a 12 meses",    color: "text-[#06D6A0]" },
  { label: "Reducción de rotación",  valor: "52% → 22%",   sub: "meta del plan de retención",       color: "text-red-400"   },
  { label: "ROI del plan",           valor: "2.8x",         sub: "por cada peso invertido en RRHH", color: "text-white"     },
];
...
const FASES = [
  ...
  { label: "Estratégico",plazo: "12 – 24 meses", dotColor: "bg-[#00B4D8]", lineColor: "border-white/10",  badgeClass: "bg-[#00B4D8]/15 text-[#00B4D8] border border-[#00B4D8]/30" },
];
...
function TooltipDestino({ active, payload, label }: {
  ...
  return (
    <div style={tooltipStyle} className="p-3 text-xs">
      ...
      {item && (
        <p className={`font-bold mt-1 ${item.premium >= 0 ? "text-[#06D6A0]" : "text-red-400"}`}>
...
export default function AnalisisTurismoPage() {
  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-[#0D2240] overflow-x-hidden">
      ...
      {/* Navbar */}
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/12"
        ...
        <div className="flex justify-between items-center max-w-7xl w-full px-6 h-16 mx-auto">
          ...
          <nav className="flex items-center gap-3">
            ...
            <a href="#cta"
              className="inline-flex items-center gap-1.5 text-sm border border-[#00B4D8]/40 text-[#00B4D8] px-4 py-1.5 rounded hover:bg-[#00B4D8]/10 transition-all duration-200 whitespace-nowrap">
              Solicitar demo
            </a>
          </nav>
        </div>
      ...
      <main className="flex-grow pt-16">
        ...
        <div className="bg-[#00B4D8]/10 border-y border-[#00B4D8]/20 py-3 px-6">
          <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <Scale size={15} className="shrink-0 text-white" />
              <p className="text-xs font-bold uppercase tracking-wider text-white">
                Ley 20.803 de Propinas vigente desde 2016 — deuda retroactiva de hasta 5 años por incumplimiento
              </p>
            </div>
            <a href="#cumplimiento" className="text-[11px] font-bold underline underline-offset-2 shrink-0 hover:opacity-80 transition-opacity text-white">
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid lg:grid-cols-5 gap-10 items-start">
              <div className="lg:col-span-2">
                <span className="text-[#00B4D8] uppercase tracking-[0.3em] mb-2 block"
                  style={{ fontFamily: "Space Mono, monospace", fontSize: "0.62rem", fontWeight: 700 }}>
                  Estructura de mercado
                </span>
                <h2 className="text-2xl font-bold text-white mb-4">Cadenas vs. Boutique vs. Independientes</h2>
                <p className="text-sm text-white/45 leading-relaxed mb-6">
                  Las cadenas internacionales pagan entre <strong className="text-white">$160.000 y $1.620.000 más</strong> que
                ...
                <div className="space-y-3">
                  {[
                    { label: "Cadena vs. independiente (chef)",  valor: "+$1.620.000 prom./mes", color: "text-[#06D6A0]" },
                    { label: "Cadena vs. independiente (recep.)",valor: "+$260.000 prom./mes",   color: "text-amber-400"  },
                    { label: "Brecha en Cam. de piso",           valor: "+$140.000 cad./indep.", color: "text-orange-400" },
                  ].map((r) => (
              ...
              <div className="lg:col-span-3 rounded-xl border border-white/10 bg-white/4 p-6">
                <div className="flex flex-wrap items-center gap-4 mb-4 text-xs">
                  {[
                    ...
                    { key: "boutique",      label: "Hotel boutique",        color: "#00B4D8" },
                ...
                <div className="h-[260px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={institucionalData} barCategoryGap="28%" barGap={3} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                      ...
                      <Bar dataKey="boutique"      fill="#00B4D8" radius={[4, 4, 0, 0]} barSize={18} />
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid lg:grid-cols-5 gap-10 items-start">
              <div className="lg:col-span-2">
                ...
                <h2 className="text-2xl font-bold text-white mb-4">El factor destino</h2>
                <p className="text-sm text-white/45 leading-relaxed mb-6">
                  El salario de un recepcionista en Isla de Pascua es un
                  <strong className="text-white"> 65% mayor</strong> que la mediana nacional del mismo cargo.
                ...
                <div className="space-y-3">
                  {[
                    ...
                    { region: "Otras regiones",  pct: -8, color: "bg-red-400"     },
                  ].map((r) => (
                    <div key={r.region} className="flex items-center gap-3 text-xs">
                      <span className="w-32 text-white/45 shrink-0">{r.region}</span>
                      <div className="flex-1 h-2 bg-white/8 rounded-full overflow-hidden">
                        <div className={`h-full ${r.color} rounded-full`} style={{ width: `${Math.max(Math.min((r.pct + 8) * 1.3, 100), 4)}%` }} />
                      </div>
                      <span className={`font-bold w-10 text-right ${r.pct >= 0 ? "text-[#06D6A0]" : "text-red-400"}`}>
                        {r.pct >= 0 ? "+" : ""}{r.pct}%
                      </span>
                    </div>
                  ))}
                ...
                <p className="text-[11px] text-white/45 mt-4 leading-relaxed bg-white/6 rounded-xl p-3">
                  Referencia: Recepcionista de hotel. Base nacional: <strong className="text-white">$680.000</strong>.
                  Fuente: ESI 2024 INE + contribuciones benchmark.
                </p>
              </div>

              <div className="lg:col-span-3 rounded-xl border border-white/10 bg-white/4 p-6">
                <p className="text-xs font-bold text-white/45 uppercase tracking-wider mb-1">Salario por destino turístico</p>
                <h3 className="text-base font-bold text-white mb-5">Recepcionista de hotel — salario bruto mensual CLP</h3>
                <div className="h-[280px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={destinoData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }} barSize={36}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.07)" vertical={false} />
                      <XAxis dataKey="region" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "rgba(255,255,255,0.35)" }} />
                      ...
                      <Tooltip content={<TooltipDestino />} cursor={{ fill: "rgba(255,255,255,0.04)" }} />
        ...
        <section className="py-24 border-t border-white/8">
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="mb-12">
              <span className="text-[#00B4D8] uppercase tracking-[0.3em] mb-3 block"
                style={{ fontFamily: "Space Mono, monospace", fontSize: "0.62rem", fontWeight: 700 }}>
                Plan de acción
              </span>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              ...
              {IMPACTO_TOTAL.map((k, i) => (
                <div key={i} className={`p-7 text-center bg-white/6 ${i < 2 ? "border-b sm:border-b-0 sm:border-r border-white/8" : ""}`}>
                    ...
                    <div className={`grid gap-5 pl-6 ${items.length === 1 ? "lg:grid-cols-2" : "md:grid-cols-2"}`}>
                        ...
                        >
                          ...
                          <div className="grid grid-cols-3 gap-3 pt-4 border-t border-white/8">
                            ...
                            <div className="text-center border-x border-white/8">
                              ...
                              <p className="text-xs font-bold text-red-400 leading-snug">{rec.evita}</p>
                            </div>
                            <div className="text-center">
                      ...
                      {items.length === 1 && (
                        <div className="border border-dashed border-white/10 rounded-lg p-6 flex flex-col items-center justify-center gap-4 text-center">
                          ...
                          <div>
                            <p className="text-sm font-bold text-white mb-1">¿Tu establecimiento tiene un perfil diferente?</p>
        ...
        <section id="cta" className="border-t border-white/8 py-24 sm:py-32">
          <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-start md:items-end justify-between gap-12">
            ...
            >
              <p className="text-[#00B4D8] uppercase tracking-[0.3em] mb-8"
                style={{ fontFamily: "Space Mono, monospace", fontSize: "0.62rem", fontWeight: 700 }}>
            ...
            <motion.a
              ...
              className="inline-flex items-center gap-2 text-sm font-semibold px-6 py-3 rounded hover:opacity-90 shrink-0"
      ...
      <footer className="border-t border-white/8 py-10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-xs text-white/35">Inteligencia salarial para Chile. Tus datos son anónimos y nunca se venderán.</p>
          </div>
          <nav className="flex items-center gap-6">
            <a href="/" className="text-xs text-white/35 hover:text-white/70 transition-colors">Para trabajadores</a>
            <a href="/empresas" className="text-xs text-white/35 hover:text-white/70 transition-colors">Vista general</a>
            <a href="/empresas/mineria" className="text-xs text-white/35 hover:text-white/70 transition-colors">Minería</a>
          </nav>
        </div>
      </footer>
    </div>
